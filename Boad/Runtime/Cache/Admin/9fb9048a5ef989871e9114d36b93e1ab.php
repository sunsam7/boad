...
<table class="table">
	<tr>
    	<td colspan="3" ><a href="/boad/Admin">返回专题列表</a></td>
    	<td colspan="3" class="text_right"><a href="/boad/Admin/Index/boadcontent/tid/<?php echo ($tid); ?>">发表内容</a></td>
    </tr>
  <tr>
    <th scope="col">ID</th>
...
<tbody id="c_body"></tbody>
  <tr>
  	...
  	<?php $__FOR_START_16082__=1;$__FOR_END_16082__=$pagecount+1;for($i=$__FOR_START_16082__;$i < $__FOR_END_16082__;$i+=1){ ?><option value="<?php echo ($i); ?>"><?php echo ($i); ?></option><?php } ?>
  	</select>
  	</td>
  </tr>
</table>
...
var dig_post_file = '/boad/Admin/Index/ajax_act/';
	...
	jQuery.get(dig_post_file, {
            ...
            if (data.status == 1) {
                for (var i = 0; i < data.data.length; i++) {
                	results_str = results_str + '<tr><td>'+data.data[i]['id']+'</td><td>'+data.data[i]['content']+'</td><td>'+data.data[i]['dig']+'</td><td>'+data.data[i]['comment_count']+'</td><td>'+data.data[i]['uptime']+'</td><td><a href="/boad/Admin/Index/boadcontent/id/'+data.data[i]['id']+'">修改</a> | <a href="javascript:void(0);" onclick="delete_data(this,'+data.data[i]['id']+');">删除</a></td></tr>';
                }
